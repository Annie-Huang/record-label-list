...
export class RecordLabelFormatUtils {
    public static getRecordLabels(inputFestivals: InputFestival[]): OutputRecordLabel[] {
            ...
            festival.bands.forEach(band => {
                ...
                if (tempRecordLabelObj[key]) {
                    ...
                    tempRecordLabelObj[key] = {
                        name: (!band.recordLabel || band.recordLabel === '') ? UNKNOWN_RECORD_LABEL_NAME : band.recordLabel,
                        band: band.name,
                        festivals: [festivalName],
                        showName: true // set to show record label name initial.
                    };
                }
        ...
        const recordLabels: OutputRecordLabel[] = Object.keys(tempRecordLabelObj).map(key => tempRecordLabelObj[key]);
